...
class Displacement(object):
    def __init__(self, vm):
        self.offset = vm.getByte()
        self.regsize = vm.getByte()
        self.multiplier = vm.getByte()

    def __repr__(self):
        try:
            if not (self.offset % 4 == 0 or self.offset % 4 == 3 and self.regsize == 1):
                print self.offset, self.regsize
                exit()
        except:
            raise
        if self.offset % 4 == 3 and self.regsize == 1:
            r = regNames[self.offset / 4][2]
        else:
            r = regNames[(self.offset / 4)][self.regsize - 1]
...
class Immediate(object):
    def __init__(self, vm):
        self.value = vm.getUInt32()
        if self.value > 0xffff0000:
            self.value -= 2 ** 32

    def __repr__(self):
        return hex(self.value).rstrip('L') if abs(self.value) >= 0x10 else '%d' % self.value

class MemoryReference(object):
    def __init__(self, vm):
        self.ops = vm.getByte()
        self.value = [vm.getOperand() for i in range(self.ops)]
        self.value = sorted(self.value, key=lambda x: 0 if isinstance(x, Displacement) else 1)

class ByteReference(MemoryReference):
    def __repr__(self):
        return 'BYTE PTR [%s]' % ' + '.join(repr(x) for x in self.value)

class UInt32Reference(MemoryReference):
    def __repr__(self):
        return 'DWORD PTR [%s]' % ' + '.join(repr(x) for x in self.value)

class Machine(object):
    def __init__(self, code):
        self.code = code
        self.offset = 0
        self.stack = len(self.code)

    def getByte(self):
        r = self.code[self.offset]
        self.offset += 1
        return r

    def getUInt32(self):
        r = self.code[self.offset + 3]
        r |= self.code[self.offset + 2] * 256
        r |= self.code[self.offset + 1] * 256 * 256
        r |= self.code[self.offset] * 256 * 256 * 256
        self.offset += 4
        return r
    ...
    def getOperand(self):
        op = self.getByte()
        if op == 1:
            return Displacement(self)
        if op == 2:
            return Immediate(self)
        if op == 3:
            return ByteReference(self)
        if op == 4:
            return UInt32Reference(self)
        pass

    def step(self, semantic=False):
        opcode = self.getByte()
        if opcode == 4: # call $op
            arg1 = self.getOperand()
            if semantic:
                offset = arg1.eval()
        ...
        if opcode == 2: # mov $op, $op
            arg1 = self.getOperand()
            arg2 = self.getOperand()

            if semantic:
                arg1.setValue(arg2.eval())
            else:
        ...
        if opcode == 8: # jmp $op
            ...
            if semantic:
                self.offset = arg1.eval()
            else:
                return 'jmp', arg1
            
        if opcode == 10: # movabs $op, $op
            arg1 = self.getOperand()
            arg2 = self.getOperand()

            if semantic:
                arg1.setValue(arg2.eval() >> 0)
            else:
                return 'movabs', arg1, arg2

        if opcode == 5: # add $op, $op
            arg1 = self.getOperand()
            arg2 = self.getOperand()

            if semantic:
                added = arg1.eval() + arg2.eval()

                self.overflowed = added >> 32 != 0

                arg1.setValue(added)
            else:
                return 'add', arg1, arg2
            
        if opcode == 18: # shr $op, $op
            arg1 = self.getOperand()
            arg2 = self.getOperand()
        ...
        if opcode == 3: # sub $op, $op
            arg1 = self.getOperand()
            arg2 = self.getOperand()

            if semantic:
                result = arg1.eval() + ((~arg2.eval()) + 1)
                self.overflowed = result >> 32 != 0
        ...
        if opcode == 6: # cmp $op, $op
            arg1 = self.getOperand()
            arg2 = self.getOperand()

            if semantic:
                self.zero = (arg1.eval() == arg2.eval())
